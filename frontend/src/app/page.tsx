        ...
        <article className={`mc ${leadClass}`}>
            ...
            {market.status === 2 && (
                <div className="mc-banner mc-banner-yes">
                    <span>🎉</span> YES won — {formatTaiko(total)} mTAIKO paid out
                </div>
            )}
            {market.status === 3 && (
                <div className="mc-banner mc-banner-no">
                    <span>💔</span> NO won — {formatTaiko(total)} mTAIKO paid out
                </div>
            )}
            {market.status === 4 && (
...
function ResolutionPanel({
    ...
    return (
        <div className="verdict">
            <div className="verdict-eyebrow">deadline passed · time to settle</div>
